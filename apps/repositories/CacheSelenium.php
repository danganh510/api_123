...
class CacheSelenium extends Component
{

    //to count driver selenium
    const PRE_SESSION_CACHE = "";
    const filePath = __DIR__ . "/../Cache/Selenium/";
    ...
    {
        $sessionId = self::PRE_SESSION_CACHE . "_selenium";
    ...
    public function setCache($count)
    {
        $sessionId = self::PRE_SESSION_CACHE . "_selenium";
        $cache = self::getBackCache();
        $cacheKey = $this->cacheKeyClients($sessionId);
        try {
            ...
            $result =  $cache->save($cacheKey, $count);
        } catch (\Exception $e) {
            return false;
        }
