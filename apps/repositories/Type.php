...
class Type extends Component
{
    ...
    public static function checkKeywordAndLang($keyword, $id, $language = "vn")
    {
        if ($language == "vn") {
            return self::checkKeyword($keyword, $id);
        } else {
            return ScTypeLang::findFirst(
                array(
                    'type_keyword = :keyword: AND type_id != :id: AND type_lang_code = :language:',
                    'bind' => array(
                        'keyword' => $keyword,
                        'id' => $id,
                        'language' => $language
    ...
    public static function getParentIdType($str = "", $parent = 0, $inputslc)
    {
        ...
        $data = $modelsManager->executeQuery(
            $sql,
            array(
                "parentID" => $parent
            )
